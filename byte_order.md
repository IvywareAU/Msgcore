...
story, since bits 0-23 are the size and 24-25 the addressing mode. VERSIONING.md §6 is that
argument; what follows here is only the byte-order half.
...
- **That last sentence is now ENFORCED on the wire rather than assumed.** `RecvP2PeerMsg` stage 2
  ...
  allocates — VERSIONING.md §6 and §9. **Reading a FILE is unaffected and deliberately so:**
...
> **Note added when this document was published with Msgcore (2026-08-13), and revised
> 2026-08-21.** The odds above assume uniformly distributed image sizes. The miss set is
> images whose size low byte falls in `0x00`–`0x03` or `0xA4`–`0xA7`, which
> means **every 256-byte-aligned size is missed, always** — 512, 1024 and 4096 all classify
> as `Legacy` rather than `Swapped`. Real image sizes cluster on exactly those round
> numbers, so the effective miss rate on realistic data is well above 1 in 32. The
> mechanism described above is unaffected; only the stated odds are optimistic.
>
> **A third range `0xA8`–`0xAB` existed for one day and is gone, which is worth recording
> because the reasoning that added it was sound and still lost.** These six bits became a
> **layout generation** field on 2026-08-20, and the first design registered a reserved
> `VBLock_SyncGen2` = `0xA8` so that the "known other generation" branch would be reachable
> and tested. Registering it obliged the swap arm to look for it in the low byte too, at
> 1/64 of this diagnosis — and it bought a named diagnostic for generation 2 and for
> nothing else. Replacing the registry with a fallback (2026-08-21) names **every** future
> generation and returns the diagnosis to 62 of 64. The lesson is the general one: an
> enumeration pays per entry and covers only what it enumerates.
>
> The claim that the size bounds check "catches essentially all of the remainder" also does
> not hold for one of the three readers: `Msgiomage_Duplicate` takes a reference with no
> length and so has no upper bound to check against. See `SECURITY.md` and
> the release-readiness register for that residual.
...
`LengthFieldPrepender` framing (`p2p_PhpHub.md:230,358`, `p2p_BrowserHub.md`).
